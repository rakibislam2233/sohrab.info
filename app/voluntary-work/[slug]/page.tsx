...
import { ArrowLeft, Calendar, Building2, MapPin, Share2, Heart } from 'lucide-react';
...
const VoluntaryDetail = () => {
  ...
  const work = {
    title: slug === 'winter-relief-2020' ? 'Winter Relief Distribution 2020' : 
           slug === 'environmental-service' ? 'Environmental Service Campaign' :
           'Health Awareness & Medical Camp',
    organization: slug === 'winter-relief-2020' ? 'Thakurgaon Govt. College Rover Scout Group' : 
                 slug === 'environmental-service' ? 'Bangladesh Scouts' : 'Voluntary Service Group',
    ...
    content: `
      ...
      <div className="bg-pink-50 p-8 rounded-2xl mb-8 border border-pink-100">
        <h3 className="text-xl font-bold mb-4 text-pink-900">Key Achievements:</h3>
    ...
    <main className="min-h-screen bg-white pb-24">
      ...
      <div className="relative h-[50vh] md:h-[60vh] w-full overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-end container mx-auto px-4 pb-12">
          ...
          <Reveal delay={0.1}>
            <div className="flex items-center gap-2 px-3 py-1 bg-pink-600 text-white text-[10px] font-bold uppercase tracking-widest rounded-full mb-4 w-fit">
          ...
          <Reveal delay={0.3}>
            <div className="flex flex-wrap items-center gap-6 text-white/70 text-sm">
              <span className="flex items-center gap-2"><Building2 size={16} className="text-pink-400" /> {work.organization}</span>
              <span className="flex items-center gap-2"><Calendar size={16} className="text-pink-400" /> {work.date}</span>
              <span className="flex items-center gap-2"><MapPin size={16} className="text-pink-400" /> {work.location}</span>
            </div>
          </Reveal>
        </div>
      </div>
