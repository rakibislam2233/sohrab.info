...
import { ArrowLeft, Calendar, MapPin, Camera, Share2, Compass } from 'lucide-react';
...
const TravelDetail = () => {
  ...
  const story = {
    title: slug === 'suryapuri-mango-tree' ? 'The Ancient Suryapuri Mango Tree' : 
           slug === 'dinajpur-temples' ? 'Historical Temples of Dinajpur' :
           'Serene Tea Gardens of Sylhet',
    destination: slug === 'suryapuri-mango-tree' ? 'Thakurgaon, BD' : 
                slug === 'dinajpur-temples' ? 'Dinajpur, BD' : 'Sylhet, BD',
        ...
        <div className="absolute inset-0 flex flex-col justify-end container mx-auto px-4 pb-12">
          ...
          <Reveal delay={0.3}>
            <div className="flex flex-wrap items-center gap-6 text-white/70 text-sm">
              <span className="flex items-center gap-2"><MapPin size={16} className="text-pink-400" /> {story.destination}</span>
              <span className="flex items-center gap-2"><Calendar size={16} className="text-pink-400" /> {story.date}</span>
              <span className="flex items-center gap-2"><Camera size={16} className="text-pink-400" /> Photography Journey</span>
            </div>
          </Reveal>
        </div>
      </div>
