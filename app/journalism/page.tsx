...
export default async function JournalismPage(){
  const articles = await prisma.article.findMany({ where: { isPublished: true }, orderBy: { publishedAt: 'desc' }, take: 12 })
  
  return (
    <main className="min-h-screen bg-white pt-32 pb-20">
      <div className="container mx-auto px-6">
        <div className="max-w-3xl mb-16">
          <h1 className="text-5xl md:text-6xl font-bold text-black mb-6">Journalism</h1>
          <p className="text-xl text-gray-500">
            Professional reporting and feature writing with a focus on agriculture, education, and current affairs in Bangladesh.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
          {articles.map((a) => (
            <Link key={a.id} href={`/journalism/${a.slug}`} className="group block">
              <article className="h-full flex flex-col">
                ...
                <h3 className="font-serif text-2xl leading-tight mb-4 group-hover:text-pink-600 transition-colors">
                  {a.title}
                </h3>
